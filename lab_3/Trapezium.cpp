...
Trapezium::Trapezium(size_t sb, size_t lb, size_t ls, size_t rs) : sm_base(sb), lg_base(lb), 
...
std::ostream& operator<<(std::ostream& os, const Trapezium& obj) {
    os << "a=" << obj.sm_base << ", b=" << obj.lg_base << ", c=" << obj.rt_side << ", d=" << obj.lf_side << std::endl;
    return os;
}
...
void Trapezium::Print() {
    std::cout << "\nSmaller base = " << sm_base << ", larger base = " << lg_base << ", left side = " <<  lf_side << ", right side = " << rt_side << std::endl;
}
